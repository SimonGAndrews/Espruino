...
| `jshEnableWatchDog` | 10<br>STM<br>NRF<br>ESP | `impl` | `M2` | `E.enableWatchdog` | `Yes` | maps to Pico SDK `watchdog_enable(...)` with RP2040 timeout clamping and pause-on-debug enabled; saved tests prove auto mode survival and manual-kick mode, but the destructive timeout-reset path is not yet scripted |
| `jshKickWatchDog` | 16<br>STM<br>NRF<br>ESP | `impl` | `M2` | `E.kickWatchdog` | `Yes` | maps to Pico SDK `watchdog_update()`; saved manual-mode test proves repeated kicks keep the watchdog alive before handing back to auto mode |
